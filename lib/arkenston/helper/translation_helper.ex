defmodule Arkenston.Helper.TranslationHelper do
  ...
  defp shrink_translation(translation) do
    translation |> Enum.reduce(%{}, fn {key, value}, trans ->
      key = atomize(key)
  ...
  def nvl_translations_map(input) do
    ...
    translations |> Enum.reduce(%{}, fn {locale, old_translation}, acc ->
  ...
  def shrink_translations_map(input) do
    ...
    new_translations = translations |> Enum.reduce(%{}, fn {locale, translation}, acc ->
  ...
  def translations_map(input) do
    input |> Enum.map(fn {locale, translation} ->
      translation = translation |> Enum.map(fn {key, value} ->
        key = atomize(key)
        value = nvl_field(value)
  ...
  def translations_from_object(object) do
    ...
    locales |> Enum.map(fn locale ->
      translation = fields |> Enum.map(fn field ->
        field = atomize(field)
  ...
  def translations_to_list(input) do
    input |> Enum.reduce([], fn {locale, translation}, acc ->
      translation = translation |> Enum.map(fn {key, value} ->
          key = atomize(key)
          value = nvl_field(value)
  ...
  def translations_from_list(input) do
    input |> Enum.reduce(%{}, fn translation, acc ->
  ...
  def merge_translations(input1, input2) do
    input1 = translations_map(if is_list(input1) do
      translations_from_list(input1)
    else
      input1
    end)
    input2 = translations_map(if is_list(input2) do
  ...
  defmacro __using__(_opts \\ []) do
    quote do
      ...
      def create_translations(attrs) do
        ...
        translations = TranslationHelper.merge_translations(new_translations, %{I18n.locale => virtual_fields_changes})
