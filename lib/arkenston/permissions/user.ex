defmodule Arkenston.Permissions.User do
  ...
  def check_permissions_for(:create, context, user, _) do
    ...
    author = Subject.get_author(~i(user.author.id)) || Subject.get_author_by(name: ~i(user.author.name))
    author = unless is_nil(author) do
      author |> Repo.preload(:created_by)
    end
    is_author_other = (~i(author.created_by.id) == ~i(current_user.id)) && !(is_nil(author) && is_nil(current_user))

    create_permissions = if is_author_other do
  ...
  def check_permissions_for(:change_author, context, user, _author) do
    ...
    change_author_permissions = unless is_self(context, user) do
      [
